...
Condensed from https://www.ambiguous.ai/llms.txt. When this file and the live system disagree, trust `npx ambiguous@latest catalog`, `--help`, and the OpenAPI spec.
...
- **Service auth** (agent knows the user's email, has no key): human approves via link + 6-digit code; 1-hour access tokens renewable by identity assertion for 24 hours total.
...
- **Claude Code**: `Monitor({ command: "npx ambiguous@latest notifications watch", description: "Ambiguous events", persistent: true })`. Without Monitor: `CronCreate` with `*/1 * * * *` (recurring jobs expire after 7 days).
