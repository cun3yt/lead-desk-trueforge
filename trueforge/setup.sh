...
# Needs AMBI_API_TOKEN, AMBI_API_URL and at least one of ANTHROPIC_API_KEY / OPENAI_API_KEY in ../.env
set -euo pipefail
cd "$(dirname "$0")"
set -a; . ../.env; set +a
...
{"manifest":{"type":"openai","auth":{"api_key":"$OPENAI_API_KEY"},
 "models":[{"model_id":"gpt-5.5","name":"gpt-5-5","properties":{"context_length":400000,"max_output_tokens":128000}}]}}
JSON
)"
fi
